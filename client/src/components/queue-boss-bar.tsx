...
export function QueueBossBar({
  ...
  if (loadingBosses) {
    return (
      <div className="flex items-center justify-center h-20 bg-gradient-to-r from-indigo-900/50 to-purple-900/50 rounded-2xl">
  ...
  return (
    <div className="bg-gradient-to-r from-indigo-900/80 to-purple-900/80 rounded-2xl p-3 space-y-2">
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          <Users className="w-4 h-4 text-white/80" />
          <span className="text-xs font-bold text-white/90 uppercase tracking-wide">Quick Queue</span>
        </div>
        <span className="text-[10px] text-white/60">Tap to join queue</span>
      </div>
        ...
        {activeBosses.map((boss) => {
          ...
          return (
            <button
              ...
              disabled={isJoining}
              className={cn(
                "flex-shrink-0 flex flex-col items-center gap-1 p-2 rounded-xl transition-all min-w-[72px]",
                inQueue 
            ...
            >
              ...
              <span className="text-[10px] text-white font-medium truncate max-w-[64px]">
                {boss.name.split(' ')[0]}
              </span>
              <span className={cn(
                "text-[8px] px-1.5 py-0.5 rounded-full font-bold",
                inQueue ? "bg-green-500 text-white" : "bg-white/20 text-white/80"
              )}>
                {inQueue ? `#${position}` : `T${boss.tier}`}
              </span>
            </button>
          );
        })}

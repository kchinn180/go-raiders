...
function generateMockLobbies(): Lobby[] {
  return Array.from({ length: 8 }).map((_, i) => ({
    ...
    hostId: `host-${i}`,
    hostName: `Raider_${Math.floor(Math.random() * 9999)}`,
    hostRating: (3.5 + Math.random() * 1.5).toFixed(1),
    players: Array.from({ length: Math.floor(Math.random() * 3) + 1 }).map(
      (_, j) => ({
        id: `p-${i}-${j}`,
        name: j === 0 ? `Raider_${Math.floor(Math.random() * 9999)}` : `Player${j}`,
        level: 30 + Math.floor(Math.random() * 20),
        team: (["ember", "frost", "storm"] as const)[j % 3],
    ...
    maxPlayers: 6,
    team: TEAMS[i % 4].id as "ember" | "frost" | "storm" | "neutral",
    minLevel: i % 3 === 0 ? 40 : 1,
